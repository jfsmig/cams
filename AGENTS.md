...
A model is a tool, not an author. The trailer that records its use is:

```
Assisted-By: <tool>
```

`Assisted-By: Claude Code`, `Assisted-By: Codex`, `Assisted-By: Copilot`. It is the last
line of the commit message, one line, and the same line goes at the end of a pull
request description. Nothing else records the assistance -- not the subject, not the
body, not the diff.

Three things it must not be:

1. **Not `Co-Authored-By:`.** Git and GitHub read that trailer as a person: it grants a
   commit a second author, and in an AGPL-3.0 project the authors are the people who
   hold the copyright. `AUTHORS` lists humans and one company for that reason. A model
   holds nothing and cannot sign a licence, so listing it as a co-author states
   something false about the provenance of the code.
2. **Not the model, only the tool.** `Claude Code`, not `Claude Opus 5 (1M context)`.
   Model names and their context-window suffixes churn every few months, so a message
   that names one is stale before the branch that carries it is old; and the tool is
   what was actually operated. The same applies to any version or build number.
3. **Not an address.** No `<noreply@anthropic.com>`, no email of any kind. An address on
   a trailer says a party can be reached about the change; nobody can be.

The rule covers **code comments as well as commit messages**. No `// generated by`, no
`// written by <model>`, no model name anywhere in a source file, a `//go:generate`
directive or a header. Source files carry the AGPL header and say nothing about who or
what typed them; generated files say which *generator* produced them from which input
-- `go/api/pb/` is the example -- which is a fact about the build, not attribution.

For Claude Code the same string is also configured mechanically, in the `attribution`
block of `~/.claude/settings.json`, so it does not depend on this file being read.
