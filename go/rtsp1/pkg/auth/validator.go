...
func NewValidator(user string, pass string, methods []headers2.AuthMethod) *Validator {
	if methods == nil {
		methods = []headers2.AuthMethod{headers2.AuthBasic, headers2.AuthDigest}
	}
	...
	if userHashed || passHashed {
		methods = []headers2.AuthMethod{headers2.AuthBasic}
	}
...
// authenticate.
func (va *Validator) Header() base2.HeaderValue {
	var ret base2.HeaderValue
	for _, m := range va.methods {
		switch m {
		case headers2.AuthBasic:
			ret = append(ret, (&headers2.Authenticate{
				Method: headers2.AuthBasic,
				Realm:  &va.realm,
			}).Marshal()...)

		case headers2.AuthDigest:
			ret = append(ret, headers2.Authenticate{
...
func (va *Validator) ValidateRequest(req *base2.Request, baseURL *url.URL) error {
	var auth headers2.Authorization
